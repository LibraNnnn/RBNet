...
def nms_with_balanced_weight(boxes, scores, labels, num_classes, weights):
    # 对 boxes 根据 scores 进行降序排列
    indices = np.argsort(-scores)
    boxes_ordered = boxes[indices]
    labels_ordered = labels[indices]
    ...
    while len(indices) > 0:
        ...
        ious = compute_iou(boxes_ordered[0], boxes_ordered[1:])
        iou_mask = np.where(ious < 0.5, 1.0, 0.0) # 将 IOU 小于阈值的框排除
        class_mask = np.where(labels_ordered[1:] == labels_ordered[0], 1.0, 0.0) # 将不同类别的框排除
        weight_mask = weights[labels_ordered[0]] / np.maximum(weights[labels_ordered[1:]], 1e-6) # 样本权重的比例

        scores_ordered = scores[indices[1:]]
        scores_ordered *= iou_mask * class_mask * weight_mask

        indices_left = np.argsort(-scores_ordered)
        indices = indices[1:][indices_left]
        boxes_ordered = boxes_ordered[1:][indices_left]
        labels_ordered = labels_ordered[1:][indices_left]
